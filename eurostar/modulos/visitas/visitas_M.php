...
class Visitas_M{
    ...
    public function createVisitM($datos){
        ...
        if ($qryresult) {
            ...
            if($muestra=='true'){
                ...
                `fk_id_prospecto`, `fk_id_visita`, `estatus`) VALUES ('$nombreMuestra', '$descripcion', '$fechaCom', '$id_vend', '$id_pros', '$visita', '1');");
                if($qryresult2){
                    return $qryresult2;
                }else{
                    return null;
    ...
    public function readMuestras(){
        ...
        INNER JOIN es_personas per ON p.fk_id_persona=per.id_persona INNER JOIN es_empresas e ON e.id_empresa=p.fk_id_empresa;");
        if($qryresult){
            return $qryresult;
        }else{
            return null;
    ...
    public function readProductos(){
        $qryresult=RunQuery("SELECT * FROM `es_tipo_productos`;");
        if($qryresult){
            return $qryresult;
        }else{
            return null;
